...
export const RFQQuoteModal: React.FC<RFQQuoteModalProps> = ({
  ...
  const handleSubmitQuote = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await marketplaceService.submitQuote(quoteRequestId, {
        offeredQuantityKg,
        offeredPricePerKg,
      });
  ...
  const handleCreateRFQ = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await marketplaceService.createQuoteRequest({
        listingId,
        deadline,
        allocationPolicy,
      });
          ...
          <form onSubmit={handleSubmitQuote} className="space-y-4 text-xs">
            ...
            <div className="flex justify-end space-x-3 pt-3">
              ...
              <button
                ...
                className="px-5 py-2 rounded-xl bg-gradient-to-r from-brand-600 to-brand-500 hover:from-brand-500 text-charcoal-950 font-bold shadow-md shadow-brand-500/20 disabled:opacity-50"
              >
